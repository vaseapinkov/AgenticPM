...
module.exports = function startServer({ port, repoName, tasksFile, boardDir }) {
  ...
  const DEFAULT_SETTINGS = {
    brainstorm:  'kiro:claude-opus-4.6',
  ...
  function runExplorationAgent(brainstormId) {
    return new Promise((resolve) => {
      ...
      const model = settings.brainstorm || 'kiro:claude-opus-4.6';
      const { type: agentType } = resolveAgent(model);
...
PATTERNS: Architectural patterns, conventions, coding style (2-3 sentences)`;
      ...
      if (isClaude) {
        ...
        if (model === 'claude:sonnet') args.push('--model', 'claude-sonnet-4-20250514');
        const env = Object.fromEntries(Object.entries(process.env).filter(([k]) => k !== 'CLAUDECODE'));
        spawnOpts = { shell: true, cwd: repoRoot, env };
      } else {
        const kiroModel = KIRO_MODEL_MAP[model] || 'claude-opus-4.6';
        cmd = 'kiro-cli';
        args = ['chat', '--no-interactive', '--trust-all-tools', '--model', kiroModel];
  ...
  function buildAlexSystemPrompt(bs) {
    ...
    const codebaseSection = ctx.explored && ctx.summary
      ? `\nCODEBASE CONTEXT (you already know this — do NOT ask the user about things you can see here):
${ctx.summary}
${ctx.techStack ? 'Tech Stack: ' + ctx.techStack : ''}
${ctx.structure ? 'Key Files:\n' + ctx.structure : ''}

IMPORTANT: You already know the tech stack, project structure, config patterns, and existing code.
Do NOT ask questions about things visible in the codebase context above (e.g. "what language?", "what framework?", "how are configs managed?").
Instead, use this knowledge to ask BETTER questions — about intent, priorities, constraints, and decisions that only the user can answer.
When writing tasks later, use this context to be specific and technical.`
      : '';
...
After the task list, the system will show import buttons automatically.`;
  ...
  async function brainstormTurn(brainstormId, userMessage) {
    ...
    // Run agent
    let output = await runBrainstormAgent(fullPrompt, brainstormId);

    // Guard against empty output (CLI not found or failed silently)
    if (!output || !output.trim()) {
      output = '{"type":"chat","content":"Sorry, I wasn\'t able to respond — the configured AI model may not be available. Please check the Brainstorm model in Settings."}';
    }
...
REJECTED: <specific what is wrong and exactly what needs fixing>`;
  ...
  function enqueueTask(taskId) {
    if (!taskQueue.includes(taskId)) {
      ...
      appendLog(taskId, `[${new Date().toLocaleTimeString('en-GB',{hour12:false})}] 📋 Queued (position ${taskQueue.length})`);
    }
    drainQueue();
  }
  ...
  async function drainQueue() {
    ...
    while (taskQueue.length > 0) {
      ...
      if (!nextId) {
        // All remaining tasks are blocked — log it
        const allTasks = load();
        taskQueue.forEach(id => {
          const t = allTasks.find(x => x.id === id);
          if (t) {
            const blockers = (t.dependencies || [])
              .map(depId => allTasks.find(x => x.id === depId))
              .filter(d => d && d.status !== 'done')
              .map(d => d.title);
            if (blockers.length) {
              appendLog(id, `[${new Date().toLocaleTimeString('en-GB',{hour12:false})}] ⏸ Waiting on: ${blockers.join(', ')}`);
            }
          }
        });
        break;
      }
      taskQueue.splice(taskQueue.indexOf(nextId), 1);
      await processTask(nextId);
      // After task completes, loop continues to pick next ready
    }
    queueRunning = false;
  }
  ...
  app.patch('/api/settings', (req, res) => {
    ...
    const u = { ...cur };
    if (VALID_MODELS.includes(req.body.brainstorm))          u.brainstorm  = req.body.brainstorm;
  ...
  app.patch('/api/tasks/:id', (req, res) => {
    ...
    if (req.body.status === 'todo' && prev !== 'todo') setTimeout(() => enqueueTask(task.id), 400);
    res.json(task);
  });
  ...
  app.post('/api/brainstorms/:id/import', (req, res) => {
    ...
    // Pass 1: create all tasks, collect title→id map for dependency resolution
    const titleToId = {};
    const newTasks = [];
    for (let ei = 0; ei < epics.length; ei++) {
      ...
      for (let ti = 0; ti < epicTasks.length; ti++) {
        ...
        const task = {
          ...
          status: 'backlog', createdAt: new Date().toISOString(),
          dependencies: [], _rawDeps: deps,
          log: '', agentOutput: '', reviewNote: '', screenshots: [],
          retryCount: 0, prUrl: null, branch: null
        };
        newTasks.push(task);
        titleToId[gt.title.toLowerCase()] = task.id;
        titleToId[task.title.toLowerCase()] = task.id;
      }
    }

    // Pass 2: resolve text dependencies to task IDs
    for (const task of newTasks) {
      const rawDeps = task._rawDeps || [];
      delete task._rawDeps;
      task.dependencies = rawDeps
        .map(depName => {
          const key = depName.toLowerCase().trim();
          if (titleToId[key]) return titleToId[key];
          // Fuzzy: find a title that contains the dep name
          const match = Object.keys(titleToId).find(t => t.includes(key) || key.includes(t));
          return match ? titleToId[match] : null;
        })
        .filter(Boolean);
      tasks.push(task);
