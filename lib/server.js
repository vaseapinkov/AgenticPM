...
module.exports = function startServer({ port, repoName, tasksFile, boardDir }) {
  ...
  function agentCmd(agentType, prompt) {
    const safe = prompt.replace(/"/g, '\\"');
    if (agentType === 'claude') return { cmd: 'claude', args: ['--print', `"${safe}"`] };
    return { cmd: 'kiro-cli', args: ['chat', '--no-interactive', '--trust-all-tools', `"${safe}"`] };
  }

  function runAgent(agentType, prompt, taskId, label) {
    return new Promise((resolve) => {
      const ts = () => new Date().toLocaleTimeString('en-GB', { hour12: false });
      const { cmd, args } = agentCmd(agentType, prompt);
      appendLog(taskId, `[${ts()}] [${label}] starting (${agentType})...`);
      const repoRoot = path.dirname(boardDir);
      const proc = spawn(cmd, args, { shell: true, timeout: 300000, cwd: repoRoot });
      let output = '';
      let hasData = false;
